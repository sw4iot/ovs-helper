...
class OVSBridge(object):
    ...
    def __init__(self, ovsdb_addr, br_name, timeout=None,
                 exception=None):
    ...
    def get_bridge_name(self, datapath_id=None):
        """ get Bridge name of a given '' """
        if datapath_id is not None:
            command = ovs_vsctl.VSCtlCommand(
            'find',
            ('Bridge',
             'datapath_id=%s' % dpid_lib.dpid_to_str(datapath_id)))
        else:
            command = ovs_vsctl.VSCtlCommand(
                'find',
                ('Bridge',
    ...
    def add_bridge(self):
        ...
        """
        command = ovs_vsctl.VSCtlCommand('add-br', (self.br_name, ))
        self.run_command([command])
    ...
    def del_bridge(self):
        ...
        """
        command = ovs_vsctl.VSCtlCommand('del-br', [self.br_name])
        self.run_command([command])
    ...
    def add_port(self, port):
        ...
        """
        command = ovs_vsctl.VSCtlCommand('add-port', (self.br_name, port))
        self.run_command([command])
    ...
    def del_port(self, port):
        ...
        :param port: port name
        """
        command = ovs_vsctl.VSCtlCommand('del-port', (self.br_name, port))
        self.run_command([command])
       
    def add_patch(self, patch0, patch1):
        options = 'peer=%s' % patch1
        args = ['Interface', patch0, 'type=patch', 'options:%s' % options]

        command_add = ovs_vsctl.VSCtlCommand('add-port', (self.br_name, patch0))
        command_set = ovs_vsctl.VSCtlCommand('set', args)
        self.run_command([command_add, command_set])

    def add_vlan(self, port, vlan):
        """ Add vlan

        :param port: port name
        :param vlan: vlan tag
        """
        args = ['Port', port, 'tag=%s' % vlan]
        command_set = ovs_vsctl.VSCtlCommand('set', args)
        self.run_command([command_set])
        
    def add_protocol(self, ofversion):
        """ Add protocol

        :param ofversion: openflow version (e.g., OpenFlow10, OpenFlow11, OpenFlow12, OpenFlow13, etc)
        """
        args = ['Bridge', self.br_name, 'protocols=%s' % ofversion]
        command_set = ovs_vsctl.VSCtlCommand('set', args)
        self.run_command([command_set])
